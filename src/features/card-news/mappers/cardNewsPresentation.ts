import type { CardNewsDisplayEntry, CardNewsItem } from '../model/cardNews';
...
type FallbackDisplayEntry = Required<
...
const fallbackDisplayEntries: FallbackDisplayEntry[] = [
  {
    id: 'peer-samsung-1',
...
function buildDisplayEntries(cards: CardNewsItem[]) {
  const withApiEntries = cards.flatMap((card) =>
    (card.displayEntries ?? []).map((entry) => ({
      ...
      peer: (entry.peerCompany ?? '삼성SDS') as PeerName,
      sector: (entry.sector ?? 'AX') as Exclude<SectorName, '섹터' | 'AI' | 'Peer'>,
      sourceType: (entry.sourceType ?? '뉴스') as SourceTypeName,
      date: entry.displayDate ?? card.date,
      accentLabel: entry.badgeLabel ?? card.category,
      coverStyle:
        entry.coverStyle ??
        'linear-gradient(180deg, rgba(37,37,40,0.55), rgba(17,17,17,0.82)), linear-gradient(135deg, #364153 0%, #111827 100%)',
      previewImageStyle:
        entry.previewImageStyle ??
        'linear-gradient(135deg, rgba(248,249,251,0.92), rgba(232,236,242,0.74)), linear-gradient(125deg, #dbe3ef 0%, #edf2f7 100%)',
    })),
  );
  ...
  return fallbackDisplayEntries.flatMap((entry) => {
