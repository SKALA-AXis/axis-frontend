...
export function GlobalTrendsPanel({ embedded = false, onUpdateTimeChange }: GlobalTrendsPanelProps) {
  ...
  const headline = runData?.final_one_liner || topItems[0]?.summary || '';
  const snapshots = runData?.snapshots ?? [];
  const detections = useMemo(() => {
    if (runData?.trend_detections && runData.trend_detections.length > 0) {
      return runData.trend_detections.slice(0, 10).map((detection) => ({
        ...detection,
        theme: detection.theme.replace(/_/g, ' '),
      }));
    }
    return buildDetectionsFromItems(listData?.items ?? []);
  }, [runData?.trend_detections, listData?.items]);
  ...
  const handleRun = async () => {
    const result = await run({ window_days: 30, max_trend_count: 10, include_peer_alignment: false });
    if (result) {
      await reload();
    }
  };
  ...
  const content = (
    <PageState
      error={error}
      empty={!isLoading && topItems.length === 0 && !runData}
          ...
          <div className="p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <ExecutiveBadge tone="accent">
                {runData?.confidence != null ? `신뢰도 ${Math.round(runData.confidence * 100)}%` : '글로벌 신호'}
              </ExecutiveBadge>
            </div>
            <p className="mt-4 text-sm font-semibold leading-7 text-[var(--axis-ink)]">
              {headline || '분석 실행 후 글로벌 6사 기준 한 줄 요약이 표시됩니다.'}
            </p>
          </div>
    ...
    <>
      ...
      {isRunning ? (
        <PageProcessLoading
          ...
          description="글로벌 6사 newsroom 기준 5단계 분석 파이프라인을 실행하고 있습니다."
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {snapshots.map((snapshot) => (
          <article key={snapshot.company_id} className="rounded-[var(--axis-radius-md)] border border-[var(--axis-hairline)] bg-[var(--axis-canvas)] p-4">
            <p className="text-sm font-semibold text-[var(--axis-ink)]">{companyLabel(snapshot.company_id)}</p>
            <p className="mt-2 text-2xl font-display font-bold text-[var(--axis-accent-strong)]">{snapshot.card_count}</p>
            <p className="text-xs text-[var(--axis-muted)]">수집 건수</p>
            {snapshot.top_themes && snapshot.top_themes.length > 0 ? (
              <p className="mt-3 text-xs leading-5 text-[var(--axis-body)]">
                {snapshot.top_themes.slice(0, 4).map((theme) => theme.replace(/_/g, ' ')).join(' · ')}
              </p>
            ) : null}
          </article>
        ))}
