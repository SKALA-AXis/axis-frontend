...
export function DonutCalloutChart({ data }: { data: DonutCalloutDatum[] }) {
  ...
  if (data.length === 1) {
    const item = data[0];
    const anchor = polarPoint(cx, cy, outerRadius + 2, 0);
    const elbow = polarPoint(cx, cy, outerRadius + 18, 0);
    const labelX = 300;
    ...
    return (
      <svg viewBox="0 0 360 236" className="h-full w-full overflow-visible" role="img" aria-label="선택 비율 도넛 차트">
        ...
        <path
          d={`M ${anchor.x} ${anchor.y} L ${elbow.x} ${elbow.y} L ${lineEndX} ${elbow.y}`}
          fill="none"
          ...
          strokeLinecap="round"
        />
        <text x={labelX} y={elbow.y - 4} textAnchor="end" className="fill-[var(--axis-ink)] text-[13px] font-bold">
          {item.name}
        </text>
        <text x={labelX} y={elbow.y + 14} textAnchor="end" className="fill-[var(--axis-muted)] text-[12px] font-semibold">
          {item.value} · 100%
        </text>
      </svg>
    );
